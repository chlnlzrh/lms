"""
Generate MDM (Master Data Management) lessons using Anthropic Claude Message Batches API.
...
Based on generate_snowflake_tune_lessons_batch_claude.py adapted for MDM curriculum.
"""
import os
import sys
import re
...
# Paths
CONTENT_STRUCTURE_PATH = "src/data/mdm/content_structure_mdm-curriculum-lesson-maps.md"
PROMPT_TEMPLATE_PATH = "LESSON_GENERATION_PROMPT_GENERIC.md"
LESSON_OUTPUT_DIR = "src/data/mdm/lessons"
BATCH_INPUT_DIR = "scripts/batch_api"
BATCH_REQUESTS_FILE = f"{BATCH_INPUT_DIR}/mdm_batch_requests.json"
BATCH_RESULTS_FILE = f"{BATCH_INPUT_DIR}/mdm_batch_results.jsonl"

# Process ALL lessons - no filtering
START_FROM_LESSON = None  # Set to None to process all remaining lessons
...
def prepare_prompt(lesson_details, prompt_template_path, content_structure_path):
    ...
    # Default metadata for MDM curriculum
    DEFAULT_AUDIENCE = "Data professionals, data stewards, and MDM practitioners"
    DEFAULT_FIRM_TYPE = "Organizations implementing Master Data Management"
    DEFAULT_INDUSTRY = "Master Data Management, data quality, data governance"
    ...
    prerequisites = lesson_details.get('LIST_PREREQUISITES', 'Basic understanding of Master Data Management concepts')
    ...
    # Replace SaaS references in the template with MDM-specific content
    formatted_prompt_mdm = formatted_prompt.replace(
        "an AI-Native SaaS Curriculum",
        "a Master Data Management (MDM) Curriculum"
    )
    formatted_prompt_mdm = formatted_prompt_mdm.replace(
        "content_structure_ai-native-saas-curriculum-lesson-maps.md",
        "content_structure_mdm-curriculum-lesson-maps.md"
    )
    formatted_prompt_mdm = formatted_prompt_mdm.replace(
        "realistic SaaS architecture patterns",
        "realistic MDM architecture patterns"
    )
    formatted_prompt_mdm = formatted_prompt_mdm.replace(
        "SaaS-specific terminology: multi-tenancy, tenant isolation, subscription models, usage-based pricing, CI/CD, and observability",
        "MDM-specific terminology: golden records, data mastering, entity resolution, data quality, stewardship workflows, and master data governance"
    )
    formatted_prompt_mdm = formatted_prompt_mdm.replace(
        "common SaaS stacks — Next.js, Postgres, Vercel, AWS, and Terraform",
        "common MDM patterns — registry, consolidation, coexistence, transaction styles, and hub architectures"
    )
    formatted_prompt_mdm = formatted_prompt_mdm.replace(
        "Languages accepted: TypeScript/Node.js, SQL, Terraform, or YAML",
        "Languages accepted: SQL, Python, XML/JSON (for data models), or YAML (for configuration)"
    )
    formatted_prompt_mdm = formatted_prompt_mdm.replace(
        "tenancy isolation",
        "domain isolation"
    ...
    prompt = f"""You are generating a polished, publication-ready lesson for a Master Data Management (MDM) Curriculum.
...
2. content_structure_mdm-curriculum-lesson-maps.md — this defines where the lesson fits in the broader curriculum.
...
def main():
    ...
    print(f"\n{'='*80}")
    print("MDM Lessons - Claude Message Batches API")
    print(f"Processing ALL remaining lessons")
    print(f"{'='*80}\n")
    ...
    # Check existing lessons (but process all for regeneration with Claude)
    generated = set()
    ...
    # Process ALL lessons (regenerating with Claude Batch API)
    remaining_lessons = lessons
    print(f"Existing lessons: {len(generated)}")
    print(f"Processing ALL {len(remaining_lessons)} lessons with Claude Batch API")
    print(f"(Will overwrite existing lessons)")
    
    # Filter to start from specified lesson if provided
