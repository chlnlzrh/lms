"""
Generate RPA (Robotic Process Automation) lessons using Anthropic Claude Message Batches API.

This script follows the 10-step process for calling claude-haiku-4-5-20251001 in Batch API mode.
Based on generate_mdm_lessons_batch_claude.py adapted for RPA curriculum.
"""
import os
import sys
import re
...
# Paths
CONTENT_STRUCTURE_PATH = "src/data/rpa/content_structure_rpa-curriculum-lesson-maps.md"
PROMPT_TEMPLATE_PATH = "LESSON_GENERATION_PROMPT_GENERIC.md"
LESSON_OUTPUT_DIR = "src/data/rpa/lessons"
BATCH_INPUT_DIR = "scripts/batch_api"
BATCH_REQUESTS_FILE = f"{BATCH_INPUT_DIR}/rpa_batch_requests.json"
BATCH_RESULTS_FILE = f"{BATCH_INPUT_DIR}/rpa_batch_results.jsonl"
...
def prepare_prompt(lesson_details, prompt_template_path, content_structure_path):
    ...
    # Default metadata for RPA curriculum
    DEFAULT_AUDIENCE = "RPA developers, process automation engineers, and business analysts"
    DEFAULT_FIRM_TYPE = "Organizations implementing Robotic Process Automation"
    DEFAULT_INDUSTRY = "Robotic Process Automation, business process automation, digital transformation"
    ...
    prerequisites = lesson_details.get('LIST_PREREQUISITES', 'Basic understanding of business process automation and RPA concepts')
    ...
    # Replace SaaS references in the template with RPA-specific content
    formatted_prompt_rpa = formatted_prompt.replace(
        "an AI-Native SaaS Curriculum",
        "a Robotic Process Automation (RPA) Curriculum"
    )
    formatted_prompt_rpa = formatted_prompt_rpa.replace(
        "content_structure_ai-native-saas-curriculum-lesson-maps.md",
        "content_structure_rpa-curriculum-lesson-maps.md"
    )
    formatted_prompt_rpa = formatted_prompt_rpa.replace(
        "realistic SaaS architecture patterns",
        "realistic RPA automation patterns"
    )
    formatted_prompt_rpa = formatted_prompt_rpa.replace(
        "SaaS-specific terminology: multi-tenancy, tenant isolation, subscription models, usage-based pricing, CI/CD, and observability",
        "RPA-specific terminology: attended bots, unattended bots, orchestrators, process mining, exception handling, and bot governance"
    )
    formatted_prompt_rpa = formatted_prompt_rpa.replace(
        "common SaaS stacks — Next.js, Postgres, Vercel, AWS, and Terraform",
        "common RPA platforms — UiPath, Automation Anywhere, Power Automate, Blue Prism, and open-source alternatives"
    )
    formatted_prompt_rpa = formatted_prompt_rpa.replace(
        "Languages accepted: TypeScript/Node.js, SQL, Terraform, or YAML",
        "Languages accepted: RPA platform-specific languages (UiPath XAML, Automation Anywhere AAL), Python, VB.NET, C#, or YAML (for configuration)"
    )
    formatted_prompt_rpa = formatted_prompt_rpa.replace(
        "tenancy isolation",
        "bot isolation"
    ...
    prompt = f"""You are generating a polished, publication-ready lesson for a Robotic Process Automation (RPA) Curriculum.
...
2. content_structure_rpa-curriculum-lesson-maps.md — this defines where the lesson fits in the broader curriculum.
...
def main():
    ...
    print(f"\n{'='*80}")
    print("RPA Lessons - Claude Message Batches API")
    print(f"Processing ALL lessons")
    print(f"{'='*80}\n")
    ...
    print(f"(Will overwrite existing lessons if any)\n")
