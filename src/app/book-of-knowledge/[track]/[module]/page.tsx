...
async function ModulePageContent({ params }: ModulePageProps) {
  ...
  // Color mapping based on track
  const trackColorMap: { [key: string]: string } = {
    ai: 'blue',
    de: 'green',
    saas: 'purple',
    sfdc: 'orange',
    ...
    rpa: 'blue',
    sales: 'yellow',
    sfdc_engineer: 'orange',
    ta: 'purple',
    viz_engineer: 'cyan',
    workato_engineer: 'indigo'
  }

  // Color mapping for consistent styling
  const getColorClasses = (color: string) => {
    const colorMap: { [key: string]: { bg: string, text: string, button: string, hover: string } } = {
      blue: { bg: 'bg-blue-100 dark:bg-blue-900', text: 'text-blue-600', button: 'bg-blue-600', hover: 'hover:bg-blue-700' },
      green: { bg: 'bg-green-100 dark:bg-green-900', text: 'text-green-600', button: 'bg-green-600', hover: 'hover:bg-green-700' },
      purple: { bg: 'bg-purple-100 dark:bg-purple-900', text: 'text-purple-600', button: 'bg-purple-600', hover: 'hover:bg-purple-700' },
      orange: { bg: 'bg-orange-100 dark:bg-orange-900', text: 'text-orange-600', button: 'bg-orange-600', hover: 'hover:bg-orange-700' },
      red: { bg: 'bg-red-100 dark:bg-red-900', text: 'text-red-600', button: 'bg-red-600', hover: 'hover:bg-red-700' },
      yellow: { bg: 'bg-yellow-100 dark:bg-yellow-900', text: 'text-yellow-600', button: 'bg-yellow-600', hover: 'hover:bg-yellow-700' },
      indigo: { bg: 'bg-indigo-100 dark:bg-indigo-900', text: 'text-indigo-600', button: 'bg-indigo-600', hover: 'hover:bg-indigo-700' },
      pink: { bg: 'bg-pink-100 dark:bg-pink-900', text: 'text-pink-600', button: 'bg-pink-600', hover: 'hover:bg-pink-700' },
      teal: { bg: 'bg-teal-100 dark:bg-teal-900', text: 'text-teal-600', button: 'bg-teal-600', hover: 'hover:bg-teal-700' },
      cyan: { bg: 'bg-cyan-100 dark:bg-cyan-900', text: 'text-cyan-600', button: 'bg-cyan-600', hover: 'hover:bg-cyan-700' },
      gray: { bg: 'bg-gray-100 dark:bg-gray-900', text: 'text-gray-600', button: 'bg-gray-600', hover: 'hover:bg-gray-700' }
    }
    return colorMap[color] || colorMap.blue
  }

  const trackColor = trackColorMap[track] || 'blue'
  const colors = getColorClasses(trackColor)
  const IconComponent = trackIcons[track] || Icons.GraduationCap

  const breadcrumbItems = [
    { label: 'Dashboard', href: '/' },
    { label: 'Book of Knowledge', href: '/book-of-knowledge' },
    { label: trackInfo.title || track, href: `/book-of-knowledge/${track}` },
      ...
      <div className="space-y-6">
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6">
          <div className="flex items-start space-x-4">
            <IconComponent className={`w-8 h-8 ${colors.text} mt-1 flex-shrink-0`} />
        ...
        <ModuleDetailsSection moduleNumber={moduleNumber} color={trackColor} track={track} />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {moduleLessons.map((lesson, index) => {
                ...
                return (
                  <Link
                    key={lesson.id}
                    href={`/book-of-knowledge/${track}/lesson/${lesson.slug}`}
        ...
        <div className="flex items-center justify-between">
          {moduleNumber > 1 ? (
            <Link
              href={`/book-of-knowledge/${track}/module-${moduleNumber - 1}`}
